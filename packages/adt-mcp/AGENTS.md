...
| Package                     | Purpose                                     |
| --------------------------- | ------------------------------------------- |
| `@abapify/adt-services`     | Shared service functions (see Policy below) |
...
## Dependencies policy: MCP wraps shared services

**MCP is a thin MCP-transport adapter over the shared service layer**
(`@abapify/adt-services`). This is a deliberate architectural decision — see
the _MCP ↔ CLI Coupling_ section in `.agents/repo-guide.md` for the
rationale.

Concrete consequences for this package:

- **Do** import service functions from `@abapify/adt-services` instead of
  re-implementing them. If logic exists in a CLI command, a tool handler
  here should delegate to the same service — never fork it. (`adt-cli`
  re-exports the same services for backwards compatibility, but depend on
  `adt-services` directly.)
...
- **Do not** depend on `@abapify/adt-cli` at runtime — commands/commander
  are a transport layer and must not be imported from MCP. `adt-cli` is a
  devDependency only, for parity tests.
- **Do not** create a dependency from `adt-services` or `adt-cli` back to
  `adt-mcp` — that would introduce a cycle. The arrow always points
  `adt-mcp → adt-services`.
